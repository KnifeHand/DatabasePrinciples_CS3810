...
INSERT INTO Employees (name, sal, deptCode)
VALUES ('Sam Mai Tai', 50000, 'HR');

INSERT INTO Employees (name, sal, deptCode)
VALUES ('James Brandy', 55000, 'HR');

INSERT INTO Employees (name, sal, deptCode)
VALUES ('Whisky Strauss', 60000, 'HR'),
       ('Romeo Curacau', 65000, 'IT'),
       ('Jose Caipirinha', 65000, 'IT'),
       ('Tony Gin and Tonic', 80000, 'SL');

INSERT INTO Employees (name, sal, deptCode)
VALUES ('Debby Derby', 85000, 'SL');

INSERT INTO Employees (name, sal, deptCode)
VALUES ('Morbid Mojito', 150000, NULL);
...
SELECT *
FROM Employees
         INNER JOIN Departments ON deptCode = code;
...
SET @jose_sal = (SELECT sal
                 FROM Employees
                 WHERE name LIKE 'Jose Caipi%');
...
FROM Employees,
     Departments
WHERE deptCode = code
  AND sal > (SELECT sal FROM Employees WHERE name LIKE 'Jose Caipi%');
...
SELECT *
FROM Employees A
         LEFT OUTER JOIN Departments B ON A.deptCode = B.code;
...
SELECT `desc` AS 'Department Description', COUNT(deptCode) AS 'Total'
FROM Employees
         INNER JOIN Departments ON deptCode = code
GROUP BY deptCode
HAVING deptCode IS NOT NULL;